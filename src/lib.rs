...
async fn main() {
    ...
    loop {
        ...
        process
            .until_closes(async {
                // Load initial info
                let (main_module_address, _main_module_size) = process.get_module_range("Pose-Win64-Shipping.exe").unwrap();
                #[cfg(debug_assertions)]
                print_message("found main module");

                // I think it's V5.5 but it works anyway. 5.5 not supported yet.
                let module = Module::wait_attach(&process, Version::V5_4, main_module_address).await;
                ...
                loop {
                    ...
                    if let Ok(player_location) = player.deref_offsets(&process, &module)
                        ...
                        // game.
                        let legless_luca: UnrealPointer<4> = UnrealPointer::new(Address::new(player_location), &[
                            "PlayerController",
                            "Pawn",
                            "leglessLuca",
                        ]);
                        ...
                        let demo_end: UnrealPointer<4> = UnrealPointer::new(Address::new(player_location), &[
                            "PlayerController",
                            "MyHUD",
                            "DemoEndScreen",
                            "bIsActive",
                        ...
                        let ability_unlock: UnrealPointer<4> = UnrealPointer::new(Address::new(player_location), &[
                            "PlayerController",
                            "MyHUD",
